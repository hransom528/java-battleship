...
public class javaBattleship {
	...
	static Table<Character> board = new Table<Character>(BOARD_WIDTH, BOARD_HEIGHT, Character.class); // Abstracts 2D array into Table class
	...
	public static void main(String[] args) {
		...
		while (playAgain) {
			...
			// While the game does not have a winner, play game
	...
	public static void printBoard() {
		...
		for (int i = 0; i < board.getySize(); i++) {
			...
			// Adds characters
			for (int j = 0; j < board.getxSize(); j++) {
				if (j > MIDLINE) { // Masks computer ships
					if (table[i][j] == '~') {
						sb.append("~");
					}
					else if (table[i][j] == 'X') {
	...
	public static void computerMove() {
		Random rnd = new Random(System.currentTimeMillis()); // Initializes random seed
		int computerXGuess, computerYGuess;
